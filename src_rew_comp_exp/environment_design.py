...
def domain_randomisation(mdp):
    ...
    for state in range(mdp.state_space.n):
        ...
        temp_mdp.set_state_transition_probabilities(
            state, mdp.demo_env[state][state_transition_idx]
        )
    return temp_mdp
    ...
    while True:
        ...
        for state in range(temp_mdp.state_space.n):
            for e_action in range(n_env_actions):
                ...
                temp_mdp.set_state_transition_probabilities(
                    state, temp_mdp.demo_env[state][e_action]
                )
                for reward_index in range(n_rewards):  # for sampled rewards
                    temp_Q_rew = np.zeros(
                        temp_mdp.action_space.n
                    )  # temporary Q values for current state
                    for action in range(temp_mdp.action_space.n):
                        temp_Q_rew[action] = temp_mdp.gamma * np.dot(
                            temp_mdp.get_transition_probabilities(state, action),
                            V[reward_index, :],
                        )
                ...
                for action in range(temp_mdp.action_space.n):
                    temp_Q_rew[action] = temp_mdp.gamma * np.dot(
                        temp_mdp.get_transition_probabilities(state, action), V_mean[:]
                    )
            ...
            temp_mdp.set_state_transition_probabilities(
                state, temp_mdp.demo_env[state][int(pol_env[state])]
            )
            ...
            for reward_index in range(n_rewards):
                for a in range(temp_mdp.action_space.n):
                    Q[reward_index, state, a] = s_reward[reward_index][
                        state
                    ] + temp_mdp.gamma * np.dot(
                        temp_mdp.get_transition_probabilities(state, a),
                        V[reward_index, :],
                    )
                V[reward_index, state] = max(Q[reward_index, state, :])
            # mean reward function
            for a in range(temp_mdp.action_space.n):
                Q_mean[state, a] = m_reward[state] + temp_mdp.gamma * np.dot(
                    temp_mdp.get_transition_probabilities(state, a), V_mean[:]
                )
...
def gradient_step(
    mdp, V, V_m, pol, pol_m, n_rewards, state, action, radius, learning_rate, delta_sa
):
    for next_state in range(mdp.state_space.n):
        ...
        for r in range(n_rewards):
            if pol[r, state, action] == 1:
                ...
                unit[next_state] = 1
                partial_grad += np.dot(
                    mdp.get_transition_probabilities(state, action) + unit, V[r, :]
                )
        ...
        if pol_m[state, action] == 1:
            ...
            unit[next_state] = 1
            partial_grad -= np.dot(
                mdp.get_transition_probabilities(state, action) + unit, V_m[:]
            )
