...
# Implementation of standard value iteration to solve mazes
def value_iteration_alt(P, reward, gamma, delta=0.001):
    state_space_n = len(reward)
    width = int(math.sqrt(state_space_n))
    action_space_n = len(P[0])

    V = np.zeros(len(reward))
    Q = np.zeros([len(reward), len(P[0])])
    pol = np.zeros([len(reward), len(P[0])])

    eff_state_space = [
        i
        for i in range(state_space_n)
        if (i % width != 0)
        and (i % width != width - 1)
        and (i > width)
        and (i < width * (width - 1))
    ...
    while True:
        ...
        for s in eff_state_space:
            for a in range(action_space_n):
                Q[s, a] = reward[s] + gamma * np.dot(P[s, a], V[:])
            V[s] = max(Q[s, :])
        if max(np.abs(V_old - V)) < delta:
            for s in eff_state_space:
                pol[s][np.argmax(Q[s, :])] = 1
    ...
    while True:
        ...
        for s in range(env.state_space.n):
            for a in range(env.action_space.n):
                Q[s, a] = policy[s, a] * (
                    env.get_reward(s)
                    + env.gamma * np.dot(env.get_transition_probabilities(s, a), V[:])
                )
            V[s] = np.sum(Q[s, :])
        if max(np.abs(V_old - V)) < delta:
            break
